...
sys.path.append("/media/nas/projects/liblsc/liblsc_dev")
...
def get_faulted_reg(ope):
    ...
    if ope["plan_done"] == True:
        for i in range(10):
            reg = "r{}".format(i)
            val = safe_int(ope[reg])
            if not np.isnan(val):
                if ( bu.to_unsigned(val, 32) != default_values[i]):
                    ret[i] = True
    return ret
...
def is_faulted_other_reg(ope):
    ...
    if i != -1:
        ...
        val = safe_int(ope[reg])
        if not np.isnan(val):
            if bu.s2u(val, 32) in default_values:
                print(ope)
                return True
    return False
...
def is_faulted_comp_other_reg(ope):
    ...
    if i != -1:
        ...
        val = safe_int(ope[reg])
        if not np.isnan(val):
            if bu.s2u(val, 32) in comp_default_values:
                if bu.s2u(val, 32) != comp_default_values[i]:
                    return True
    return False
...
def is_add_with_other_reg(ope):
    ...
    if i != -1:
        ...
        if not np.isnan(val):
            for j in range(10):
                if bu.s2u(val, 32) == bu.s2u(default_values[i] + default_values[j], 32):
                    return True
    return False
...
def is_and_with_comp_other_reg(ope):
    ...
    if i != -1:
        ...
        if not np.isnan(val):
            for j in range(10):
                if bu.s2u(val, 32) == bu.s2u(default_values[i] & comp_default_values[j], 32):
                    return True
    return False
...
def is_or_2_other_reg(ope):
    ...
    if i != -1:
        ...
        if not np.isnan(val):
            for j in range(10):
                for k in range(10):
                    if bu.s2u(val, 32) == bu.s2u(default_values[j] | default_values[k], 32):
                        if (i == j) or (i == k):
                            return False
                        return True
    return False
...
def is_minus_one(ope):
    ...
    if i != -1:
        ...
        val = safe_int(ope[reg])
        if not np.isnan(val):
            if bu.s2u(val, 32) == default_values[i]-1:
                return True
    return False
...
def is_cmp_bypass(ope):
    ...
    if i != -1:
        ...
        val = safe_int(ope[reg])
        if not np.isnan(val):
            if (i == 9) and (bu.s2u(val,32) == cmp_bypass_v):
                return True
    return False
...
def print_bin_diff_faulted_reg(ope):
    ...
    if i != -1:
        ...
        val = safe_int(ope[reg])
        if not np.isnan(val):
            print("0x{:08x} -> 0x{:08x}".format(default_values[i], int(bu.s2u(val, 32))))
            bu.bin_diff( default_values[i], bu.s2u(val, 32), 32)
...
add_style(to_plot, matrix_style)
pl = plotter.Plotter(to_plot, figsuptitle="")
pl.show()
#pl.export_tikz("carto_4040_faults.tex")
